...
main.py
-------
Aplikasi web Student Placement Prediction menggunakan Streamlit.
Muat model .pkl dari artifacts/ untuk prediksi klasifikasi (placement)
dan regresi (salary estimate).
...
# ---------------------------------------------------------------------------
# Custom CSS – desain industrial/utilitarian dengan aksen amber
# ---------------------------------------------------------------------------
st.markdown("""
<style>
...
# ---------------------------------------------------------------------------
# Feature engineering (mirip pre_processing.py)
# ---------------------------------------------------------------------------
...
# Status model di header
col_s1, col_s2, col_s3 = st.columns(3)
with col_s1:
    status_clf = "✅ Loaded" if clf_model else "⚠️ Not Found"
...
with col_s2:
    status_reg = "✅ Loaded" if reg_model else "⚠️ Not Found"
...
# ---------------------------------------------------------------------------
# Simpan input row untuk visualisasi
# ---------------------------------------------------------------------------
input_row = {
    'gender': gender,
    'branch': branch,
...
# ---------------------------------------------------------------------------
# Tab: Prediction vs Profile
# ---------------------------------------------------------------------------
tab1, tab2, tab3 = st.tabs(["🎯  Prediction Result", "📊  Student Profile", "ℹ️  How It Works"])
...
with tab1:
    if not predict_btn:
        ...
        if clf_model is None and reg_model is None:
            ...
            # ---- Regresi (hanya jika predicted Placed) ----
            st.markdown("---")
...
with tab2:
    ...
    with col_stats:
        ...
        risk = "⚠️ Ada Backlog" if backlogs > 0 else "✅ Bersih"
